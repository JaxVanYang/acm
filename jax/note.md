...
2. `prev()` 和 `next()` 函数
    ...
    `prev` 的原意是上一个，所以返回的是距指定迭代器前 n 个位置的迭代器，原型如下：
...
4. 读取字符的时候需要小心空白字符
    ...
    `scanf("%c", &ch)` 和 `getchar()` 都不会忽略空白字符，使用 `cin >> ch` 可以忽略空白字符，也可以采用[2. 技巧](#2-技巧)中的“使用 `scanf()` 读取字符串代替读取字符”。
